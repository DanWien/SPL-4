...
class Dao(object):
    ...
    def update(self, update_dict):
        column_names = [key for key in update_dict.keys() if key != 'id']
        set_columns = ','.join([f"{key}=?" for key in column_names])
        params = [value for key, value in update_dict.items() if key != 'id']
        stmt = 'UPDATE {} SET {} WHERE id=?'.format(self._table_name, set_columns)
        self._conn.execute(stmt, params+[update_dict['id']])
